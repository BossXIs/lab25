...
ostream& operator<<(ostream &os,const ComplexNumber &c){
	if(c.real == 0.0 && c.imag == 0.0) return os << c.real;
	else if(c.real != 0 && c.imag > 0) return os << c.real << "+" << c.imag << "i";
	else if(c.real > 0.0 && c.imag > 0.0) return os << c.real << "+" << c.imag << "i";
	else if(c.real > 0.0 && c.imag < 0.0) return os << c.real << c.imag << "i";
	else if(c.real < 0.0 && c.imag < 0.0) return os << c.real << c.imag << "i";
	else if(c.real == 0.0 && c.imag > 0.0) return os << c.imag << "i";
	else if(c.real == 0.0 && c.imag < 0.0) return os << c.imag << "i";
	else if(c.real > 0.0 && c.imag == 0.0) return os << c.real;
	else if(c.real < 0.0 && c.imag == 0.0) return os << c.real;
